...
def run_forward_test(model_path: str, test_data_path: str, session: str):
    ...
    while not done:
        ...
        if action_name != "HOLD":
            ...
            print(f"[LIVE STREAM] Candle {current_step} | Action: {action_name:<5} | Pos: {position:>2} | Equity: ${current_equity:.2f}")
            ...
            elif action_name == "CLOSE" or (action_name in ["BUY", "SELL"] and active_trade_entry_step is not None):
    ...
    print(f"Net Realized PnL:   ${net_profit:,.2f} ({(net_profit/initial_balance)*100:.2f}%)")
    print("-" * 40)
    print(f"Total Trades Taken: {total_trades}")
    print(f"Win Rate:           {win_rate:.2f}%")
    print(f"Avg Holding Time:   {avg_hold:.1f} candles ({avg_hold * 15 / 60:.2f} hours)")
    ...
    plt.fill_between(range(len(equity_curve)), initial_balance, equity_curve, where=(np.array(equity_curve) >= initial_balance), color='green', alpha=0.1)
    plt.fill_between(range(len(equity_curve)), initial_balance, equity_curve, where=(np.array(equity_curve) < initial_balance), color='red', alpha=0.1)
    plt.title(f"XAUUSD AI Strategy Forward Test (5M Steps, Tight Spread: $0.05)")
